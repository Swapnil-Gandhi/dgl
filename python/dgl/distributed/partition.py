...
from ..transform import metis_partition_assignment, partition_graph_with_halo, part_graph_with_halo, shuffle_graph_with_halo
...
def partition_graph(g, graph_name, num_parts, out_path, num_hops=1, part_method="metis",
                    reshuffle=True, balance_ntypes=None, balance_edges=False, feature_override=False, feature_len=0, part_override=False, seed=16):
    ...
    * `balance_edges` balances the number of edges in each partition.
    ...
    ...                                 'output/test.json', 0)
    '''
    if not part_override or num_parts == 1:
        if feature_override: print('[OVERRIDE] Dummy features of length {} will be generated'.format(feature_len))
        if num_parts == 1:
            parts = {0: g}
            ...
            if num_parts > 1:
                ...
                for name in g.ndata:
                    if name in [NID, 'inner_node']:
                        continue
                    if feature_override and name in ['features', 'feat']: #Should dummy features be created ?
                        print('[OVERRIDE] Generating Feature Vector of shape [{},{}]'.format(len(local_nodes), feature_len))
                        node_feats[name] = F.uniform((len(local_nodes), feature_len ), F.float32, F.cpu(), 0.0, 10.0)
                    else:
        ...
        '''
            @Swapnil:
            In an attempt to minimize memory footprint, we choose to disable OpenMP acceleration.
            Following code processes and serializes one subgraph partition at-a-time, allowing us
             to reduce memory footprint.
            NOTE: Some generality of code and performance is sacrified to reduce memory footprint.
        '''
        if part_override : print('[OVERRIDE] OpenMP acceleration is disabled')
        if feature_override : print('[OVERRIDE] Dummy features of length {} will be generated'.format(feature_len))
        if not reshuffle :
            raise Exception('Reshuffling is disabled')
        ...
        g_shuffled, orig_nids, orig_eids = shuffle_graph_with_halo(g, node_parts)
        ...
        for part_id in range(num_parts):
            # Process one subgraph partition at-a-time
            part = part_graph_with_halo(g_shuffled, node_parts, part_id, num_hops, orig_nids, orig_eids, reshuffle=reshuffle)[0]
            node_map_val.insert(part_id, F.as_scalar(F.sum(F.astype(part.ndata['inner_node'], F.int64),0)))
            edge_map_val.insert(part_id, F.as_scalar(F.sum(F.astype(part.edata['inner_edge'], F.int64),0)))
            ...
            edge_feats = {}
            local_edge = F.boolean_mask(part.edata[edata_name], part.edata['inner_edge'])
            for name in g.edata:
                if name in [EID, 'inner_edge']:
                    continue
                edge_feats[name] = F.gather_row(g.edata[name], local_edge)
            local_edge_len = len(local_edge)
            local_edge = None
            save_tensors(edge_feat_file, edge_feats)
            edge_feats = {}
            ...
            node_feats = {}
            save_graphs(part_graph_file, [part])
            local_node = F.boolean_mask(part.ndata[ndata_name], part.ndata['inner_node'])
            part = None 
            for name in g.ndata:
                if name in [NID, 'inner_node']:
                    continue
                if feature_override and name in ['features', 'feat']: #Should dummy features be created ?
                    print('[OVERRIDE] Generating Feature Vector of shape [{},{}]'.format(len(local_node), feature_len))
                    node_feats[name] = F.uniform((len(local_node), feature_len ), F.float32, F.cpu(), 0.0, 10.0)
                else:
                    node_feats[name] = F.gather_row(g.ndata[name], local_node)
            local_node = None
            save_tensors(node_feat_file, node_feats)
            print('Partition {} saved to disk'.format(part_id))
            node_feats = {}

        # orig_nids/eids are no longer needed, Free Memory
        g_shuffled = None
        orig_nids = None
        orig_eids = None
        ...
        edge_map_val = np.cumsum(edge_map_val).tolist()
        assert edge_map_val[-1] == g.number_of_edges()
        part_metadata['node_map'] = node_map_val
        part_metadata['edge_map'] = edge_map_val
